...
if __name__ == '__main__':
    ...
    #加载数据
    data = pd.read_csv('big_new_data.csv')    
    
    #得到关键词字典
    word_dic = words_dictionary(data)
    #保存字典以备后用
    f = open('new_word_dic.txt', 'w')
    ...
    np.save('big_labels.npy', labels) #保存label.npy文件，训练样本的标记
    ...
    np.save('big_features.npy', features) #保存features.npy文件，训练样本的特征      
